// Populates Holdings and Positions with the same sample data the original
// tutorial project had, so the dashboard has something to show right after
// setup. Run with: npm run seed
...
async function seed() {
  if (!uri) {
    console.error("Could not seed: MONGO_URL is not set. Copy .env.example to .env and fill in MONGO_URL.");
    process.exit(1);
  }
  ...
  console.log(`Seeded ${holdings.length} holdings and ${positions.length} positions.`);
